...
class ZFDebug_Controller_Plugin_Debug_Plugin_File implements ZFDebug_Controller_Plugin_Debug_Plugin_Interface
{
    ...
    public function getPanel()
    {
        ...
        $html = '<h4>Informations sur les fichiers</h4>';
        $html .= count($included).' fichiers inclus<br />';
        $size = 0;
        foreach ($included as $file) {
            $size += filesize($file);
        }
        $html .= 'Poids total: '. round($size/1024, 1).'K<br />';
        
        $html .= 'Chemin de de base (Basepath) : ' . $this->_basePath . '<br />';

        $libraryFiles = array();
        foreach ($this->_library as $key => $value) {
            if ('' != $value) {
                $libraryFiles[$key] = '<h4>Fichiers de la librairie ' . $value . ' </h4>';
            }
        }

        $html .= '<h4>Fichiers de l\'application</h4>';
        foreach ($included as $file) {
