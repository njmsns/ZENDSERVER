...
class ZFDebug_Controller_Plugin_Debug_Plugin_Exception implements ZFDebug_Controller_Plugin_Debug_Plugin_Interface
{
    ...
    public function getPanel ()
    {
        ...
        foreach ($response->getException() as $e) {
            ...
            $html .= '<h4>' . $e->getMessage() . '</h4><p>Levée dans ' . $e->getFile() . ' à la ligne ' . $e->getLine() . '</p>';
            $html .= '<h4>Pile d\'appels</h4><ol>';
            foreach ($e->getTrace() as $t) {
                ...
                if (! isset($t['line']))
                    $t['line'] = 'n/a';
                $html .= '<li>' . $func . '<br>dans ' . str_replace($_SERVER['DOCUMENT_ROOT'], '', $t['file']) . ' à la ligne ' . $t['line'] . '</li>';
            }
            $html .= '</ol>';
        }
        ...
        if ($errorCount) {
            ...
            foreach (self::$errors as $error) {
                $html .= '<li>' . sprintf("%s: %s dans %s à la ligne %d", $error['type'], $error['message'], str_replace($_SERVER['DOCUMENT_ROOT'], '', $error['file']), $error['line']) . '</li>';
            }
            $html .= '</ol>';
        }
        return $html;
    ...
    public static function errorHandler ($level, $message, $file, $line)
    {
        ...
        if (ini_get('log_errors'))
            error_log(sprintf("%s: %s dans %s à la ligne %d", $type, $message, $file, $line));
        return true;
    }
}